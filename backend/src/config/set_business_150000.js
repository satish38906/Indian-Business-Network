...
async function setBusinessClosedTo150000() {
  try {
    ...
    // Update received referrals to 'closed' status to reach ₹150,000
    await pool.query(
      `UPDATE referrals 
       SET status = 'closed' 
       WHERE to_member_id = $1 AND status = 'accepted'`,
      [memberId]
    );
    
    console.log('✓ Updated referrals to closed status');
